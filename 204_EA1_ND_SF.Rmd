...
weights <- data.frame(runif(172, 0, 1))
weight.random <- data.frame()
survival_weight <- data.frame()

shark_suit_df$cba <- (weights*shark_suit_df$shark_suitability + (1-weights)*shark_suit_df$tuna_suitability)/shark_suit_df$cost_of_pixel

for (i in 1:100) {
  weight.random = shark_suit_df[sample(1:nrow(shark_suit_df)),] %>%
       mutate("cum_sum" = cumsum(cost_of_pixel)) %>%
       filter(cum_sum <200) 
  
  survival_weight[i,1] = alpha_shark*(sum(weight.random$shark_suitability)^0.6)
  survival_weight[i,2] = alpha_tuna*(sum(weight.random$tuna_suitability)^0.6)

}

# survival_weight <- function (probs) {
#   
#   shark_suit_df$cba <- (probs*shark_suit_df$shark_suitability + (1-probs)*shark_suit_df$tuna_suitability)/shark_suit_df$cost_of_pixel
#   
#   x <- shark_suit_df %>%
#     arrange(-cba) %>%
#     mutate(cum_sum = cumsum(cost_of_pixel)) %>%
...
# weight <- seq(0,1,0.01)
# 
# solution <- lapply(weight, survival_weight)
# 
# df <- data.frame()
